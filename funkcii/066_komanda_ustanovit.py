async def cmd_set(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    conn.close()
    if not receptions:
        await update.message.reply_text("Приёмки не настроены. Сначала используйте /num в нужной группе.")
        return
    ...
    await update.message.reply_text("Выберите приёмку для этой темы:", reply_markup=InlineKeyboardMarkup(keyboard))
