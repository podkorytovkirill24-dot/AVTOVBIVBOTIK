async def cmd_num(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    conn.close()
    if not tariffs:
        await update.message.reply_text("Нет тарифов. Сначала добавьте тариф в админке.")
        return
    keyboard = []
    ...
    await update.message.reply_text("Выберите тариф для этой приёмки:", reply_markup=InlineKeyboardMarkup(keyboard))
