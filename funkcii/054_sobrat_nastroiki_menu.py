def build_settings_menu(conn: sqlite3.Connection) -> InlineKeyboardMarkup:
    stop = "⛔ Stop-Work: ВКЛ" if get_config_bool(conn, "stop_work") else "✅ Stop-Work: ВЫКЛ"
    auto_slip_minutes = get_config_int(conn, "auto_slip_minutes", 15)
    auto_slip = (
        f"⚠ Авто-слёт ({auto_slip_minutes} мин): "
        + ("ВКЛ" if get_config_bool(conn, "auto_slip_on") else "ВЫКЛ")
    )
    repeat_submit = (
        "🔁 Повторы: " + ("разрешены" if get_config_bool(conn, "allow_repeat") else "запрещены")
    )
    issue_dept = (
        "🧭 Выдача по отделам: " + ("ВКЛ" if get_config_bool(conn, "issue_by_departments") else "ВЫКЛ")
    )
    lunch = "🍽 Обед: " + ("ВКЛ" if get_config_bool(conn, "lunch_on") else "ВЫКЛ")
    iam_minutes = get_config_int(conn, "i_am_here_minutes", 10)
    iam_here = (
        f"✅ Я тут ({iam_minutes} мин): "
        + ("ВКЛ" if get_config_bool(conn, "i_am_here_on") else "ВЫКЛ")
    )
    input_type = "📥 Вход: приоритеты" if get_config_bool(conn, "use_priorities", True) else "📥 Вход: FIFO"
    referral = (
        "🤝 Рефералка: " + ("ВКЛ" if get_config_bool(conn, "referral_enabled", True) else "ВЫКЛ")
    )
    return InlineKeyboardMarkup(
        [
            ...
            [
                InlineKeyboardButton("📌 Лимит в день", callback_data="adm:limit"),
                InlineKeyboardButton(iam_here, callback_data="adm:i_am_here"),
            ],
            [
                InlineKeyboardButton("🏷 Приёмки (/num)", callback_data="adm:departments"),
                InlineKeyboardButton("🏢 Офисы (/set)", callback_data="adm:offices"),
            ],
            [
