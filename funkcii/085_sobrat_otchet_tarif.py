...
def _duration_info(
    ...
    start_ts = row["stood_at"] or (row["completed_at"] if row["status"] == "success" else None)
    ...
    if row["status"] == "success":
        end_value = min(now_value, end_ts) if end_ts else now_value
    else:
...
def build_report_csv(rows: List[sqlite3.Row], end_ts: Optional[int] = None) -> str:
    ...
    for r in rows:
        ...
        stood_at = r["stood_at"] or (r["completed_at"] if r["status"] == "success" else None)
